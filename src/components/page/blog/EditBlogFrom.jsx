...
export default function EditBlogFrom({ blogData }) {
  ...
  const initialValue = {
    ...
    pictureImageTitle: blogData?.banner?.title,
    category: blogData?.category,
    metaTitle: blogData?.seoContent?.metaTitle,
    metaDescription: blogData?.seoContent?.metaDescription,
  };

  const [formData, setFormData] = useState(initialValue);
  const [liveimg, setLiveImg] = useState(blogData?.banner?.image);
  const [imgError, setImgError] = useState(false);

  const [
    updateData,
    { isError, isLoading, isSuccess, error, data: addBlogData },
  ...
  useEffect(() => {
    ...
    if (isSuccess) {
      ...
      // ✅ Optionally update local state with latest info
      if (addBlogData?.blog) {
        const updatedBlog = addBlogData.blog;
        setFormData({
          title: updatedBlog.title,
          sub_title: updatedBlog.sub_title,
          slug: updatedBlog.slug,
          content: updatedBlog.content,
          pictureImageTitle: updatedBlog.banner?.title,
          category: updatedBlog.category,
          metaTitle: updatedBlog.seoContent?.metaTitle,
          metaDescription: updatedBlog.seoContent?.metaDescription,
        });
        setLiveImg(updatedBlog.banner?.image);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    let imagUrl = blogData.banner?.image;
    let formDataToSubmit = new FormData(); // Correctly naming the variable
    ...
    await updateData({ blogId: blogData?._id, data: formDataToSubmit });
  };
        ...
        <div className="sm:p-7 p-4 rounded-md xl:w-9/12 mt-5 xl:mt-0  bg-[#0B1315] border border-[#142225]  relative overflow-hidden">
          ...
          <div className="mt-5 w-full z-[6] relative">
            ...
            <select
              ...
              className="mt-2 w-full p-[13px] rounded-md border border-[#375861] outline-none text-gray-0 bg-transparent text-[15px]"
              required
            >
              {categories?.map((categ) => (
                <option
                  key={categ?._id}
                  value={categ?._id}
                  selected={categ?._id == blogData?.category}
                  className="cursor-pointer text-gray-0 bg-[#375861]"
                >
                  {categ?.title}
                </option>
              ))}
